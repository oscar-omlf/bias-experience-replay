...
class TabularDynamicsModel:
    ...
    @staticmethod
    def _outcome_key(s_next: int, r: float, terminated: bool, truncated: bool) -> Tuple[int, float, bool, bool]:
        # Match replay storage: rewards are stored as float32 before later removal.
        return (int(s_next), float(np.float32(r)), bool(terminated), bool(truncated))
    ...
    def observe(
        ...
        key = (int(s), int(a))
        z = self._outcome_key(s_next=s_next, r=r, terminated=terminated, truncated=truncated)
    ...
    def unobserve(
        ...
        z = self._outcome_key(s_next=s_next, r=r, terminated=terminated, truncated=truncated)
        old = int(counts_dict.get(z, 0))
        if old <= 1:
            counts_dict.pop(z, None)
        else:
            counts_dict[z] = old - 1

        if not counts_dict:
            self.outcome_counts.pop(key, None)

    def count(self, s: int, a: int) -> int:
        counts_dict = self.outcome_counts.get((int(s), int(a)), None)
        if not counts_dict:
            return 0
        return int(sum(counts_dict.values()))
