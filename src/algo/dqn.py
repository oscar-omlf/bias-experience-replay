...
class DoubleDQN:
    def __init__(
        ...
        device: str = "cpu",
        handle_time_limit_as_terminal: bool = True,
    ):
        self.q_net = q_net
        ...
        self.device = device
        self.handle_time_limit_as_terminal = bool(handle_time_limit_as_terminal)
    ...
    def compute_loss(self, batch: Dict, optimizer) -> Tuple[torch.Tensor, Dict]:
        ...
        # IMPORTANT: set_to_none=True avoids extra memset and can be faster on GPU
        optimizer.zero_grad(set_to_none=True)
        loss.backward()

        grad_norm = None
        if self.max_grad_norm and self.max_grad_norm > 0:
            gn = torch.nn.utils.clip_grad_norm_(self.q_net.parameters(), self.max_grad_norm)
            grad_norm = float(gn.item())  # this is a sync, but unavoidable if you want the value

        optimizer.step()

        with torch.no_grad():
            td_errors = (q_sa - target).detach()
            td_error_mean = td_errors.abs().mean().detach()
            q_mean = q_values.mean().detach()

        logs = {
            "loss": loss.detach(),                 # 0-d tensor
            "td_errors": td_errors,                # (B,) tensor
            "td_error_mean": td_error_mean,         # 0-d tensor
            "q_mean": q_mean,                       # 0-d tensor
